...
then
    ...
    echo -e ${GREEN}'* Changing password hash algorithem : SHA512 \n'${NC}
    ...
    echo -e ${GREEN}'* Add sshd to /etc/hosts.allow & deny other services \n'${NC}
...
#############################################
    ...
    print -e ${GREEN}'Disabling USB cooldisk/Flash mount'${NC}
    /sbin/grubby --update-kernel=ALL --args="nousb"
    sleep 5

    print -e ${INVERT}' Script will enable SELinux for changing configs.'${NC}
